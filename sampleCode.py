...
dataFolder = "/home/garrett/kaggle/russianAds/"
...
#	joblib.dump((trainFeatures, trainTargets, trainItemIds, testFeatures, testItemIds), os.path.join(dataFolder,"train_data.pkl"))
	...
	f_val = open('Validation_Results.txt', 'a')
	f_val.write("\n")
	t_size = .97

	#clf = SGDClassifier(    loss="log, penalty="l2", n_iter= 10, alpha=1e-5)#,class_weight="auto")
	clf = LogisticRegression(penalty="l2", C=1)
	train, val, t_labs,v_labs = train_test_split(trainFeatures, trainTargets, train_size = t_size, random_state= 33)
	clf.fit(train,t_labs)
	val_score = clf.score(val, v_labs)
	print "Validation Score: ", val_score
	f_val.write("{0}, {1}, {2}".format(train.shape[0], val.shape[0], val_score))
	f_val.write("\n")
	#clf.predict(trainFeatures)
